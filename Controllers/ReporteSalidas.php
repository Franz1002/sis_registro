<?php 
	...
	class ReporteSalidas extends Controllers{
		public $views;
		public $model;
		...
		{
			$data['page_tag'] = "REPORTES DE SALIDAS";
			$data['page_title'] = "REPORTE DE ARCHIVO";
			$data['page_functions_js'] = "functions_reporteS.js";
			$this->views->getView($this,"reporteSalidas",$data);
		}

		public function reporteTotal()
		{
			$data = $this->model->getSalidaTotal();
			...
			$fpdf->Cell(50, 10, 'REPORTE TOTAL DE SALIDAS DE DOCUMENTOS', 0, 1, 'C');
			...
			$fpdf->Cell(35, 10, 'DESTINATARIO', 1, 0, 'C', true);
			$fpdf->Cell(33, 10, 'FECHA SALIDA', 1, 0, 'C', true);
			...
			foreach ($data as $columna) {
				...
				$fpdf->Cell(12);
				$fpdf->Cell(8, 15, $columna['id_salida'], 1, 0, 'C', true);
				...
				$fpdf->Cell(35, 15, $columna['destinatario_salida'], 1, 0, 'C', true);
				$fpdf->Cell(33, 15, $columna['fecha_salida'], 1, 0, 'C', true);
		...
		public function reporteInformeS()
		{
			$data = $this->model->getSalidaInforme();
			...
			$fpdf->Cell(0, 10, 'REPORTE DE SALIDAS POR USUARIO', 0, 1, 'C');
			$fpdf->Ln(4);
		...
		{
			$fechaDesde = date('Y-m-d', strtotime($_POST['fechasalidaD']));
			$fechaHasta = date('Y-m-d', strtotime($_POST['fechasalidaH']));
			$listArea = $_POST['reporteArea'];
			$data = $this->model->getSalidaArea($listArea, $fechaDesde, $fechaHasta);
			...
			$fpdf->SetFont('Arial', 'B', 12);
			$fpdf->SetTextColor(0, 0, 0);
			$fpdf->Cell(50, 10, utf8_decode('REPORTE TOTAL POR ÁREA DE SALIDAS DE DOCUMENTOS'), 0, 1, 'C');
			$fpdf->SetDrawColor(61, 174, 233);
			$fpdf->SetLineWidth(2.5);
		...
		public function reporteFechaS()
		{
			...
			$fechaDesde = date('Y-m-d', strtotime($_POST['fechasalidaD']));
			$fechaHasta = date('Y-m-d', strtotime($_POST['fechasalidaH']));
			$data = $this->model->getSalidaFecha($fechaDesde, $fechaHasta);
			...
			$fpdf->SetFont('Arial', 'B', 12);
			$fpdf->SetTextColor(0, 0, 0);
			$fpdf->Cell(50, 10, 'REPORTE TOTAL POR RANGO DE FECHAS DE SALIDAS DE DOCUMENTOS', 0, 1, 'C');
			$fpdf->SetDrawColor(61, 174, 233);
			$fpdf->SetLineWidth(2.5);
